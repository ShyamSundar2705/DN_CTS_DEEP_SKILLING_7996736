...
export const enrollmentReducer = createReducer(
  ...
  on(enrollInCourse, (state, { courseId }): EnrollmentState => ({
    ...state,
    enrolledCourseIds: state.enrolledCourseIds.includes(courseId)
      ? state.enrolledCourseIds
      : [...state.enrolledCourseIds, courseId],
  })),
  on(unenrollFromCourse, (state, { courseId }): EnrollmentState => ({
    ...state,
    enrolledCourseIds: state.enrolledCourseIds.filter(id => id !== courseId),
  })),
  on(setEnrolledCourses, (state, { courseIds }): EnrollmentState => ({
    ...state,
